...
void ReportResults(const SparseMatrix &A, int numberOfMgLevels,
                   ...
                   std::vector<TestResult> results, bool quickPath) {

  double minOfficialTime =
      1800; // Any official benchmark result must run at least this many seconds
  ...
  if (A.geom->rank ==
    ...
    for (int i = 1; i < numberOfMgLevels; ++i) {
      ...
      fnwrites_precond +=
          fniters * fnnz_Af *
          sizeof(double); // Number of writes for fine grid residual calculation
      fnreads_precond +=
    ...
    if (isValidRun) {
      ...
      if (!A.isMgOptimized) {
        if (A.geom->numThreads > 1)
          doc.get("Final Summary")
              ->add("Reference version of ComputeMG used and number of threads "
                    "greater than 1",
      ...
      if (times[0] >= minOfficialTime) {
        ...
        if (quickPath) {
          ...
          doc.get("Final Summary")
              ->add("After confirmation please upload results from the YAML "
                    "file contents to",
                    "http://hpcg-benchmark.org");
        } else {
          doc.get("Final Summary")
