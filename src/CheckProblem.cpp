...
void CheckProblem(SparseMatrix &A, Vector *b, Vector *x, Vector *xexact) {
  ...
  global_int_t totalNumberOfRows =
      gnx * gny * gnz; // Total number of grid points in mesh
  ...
  for (local_int_t iz = 0; iz < nz; iz++) {
    ...
    for (local_int_t iy = 0; iy < ny; iy++) {
      ...
      for (local_int_t ix = 0; ix < nx; ix++) {
        ...
        HPCG_fout << " rank, globalRow, localRow = " << A.geom->rank << " "
                  << currentGlobalRow << " "
                  << A.globalToLocalMap.find(currentGlobalRow)->second << endl;
#endif
        char numberOfNonzerosInRow = 0;
        double *currentValuePointer =
            A.matrixValues[currentLocalRow]; // Pointer to current value in
                                             // current row
        global_int_t *currentIndexPointerG =
            A.mtxIndG[currentLocalRow]; // Pointer to current index in current
                                        // row
                ...
                for (int sx = -1; sx <= 1; sx++) {
                  if (gix + sx > -1 && gix + sx < gnx) {
                    ...
                    if (curcol == currentGlobalRow) {
                      assert(A.matrixDiagonal[currentLocalRow] ==
                             currentValuePointer);
...
#ifdef HPCG_DETAILED_DEBUG
  HPCG_fout << "Process " << A.geom->rank << " of " << A.geom->size << " has "
            << localNumberOfRows << " rows." << endl
            << "Process " << A.geom->rank << " of " << A.geom->size << " has "
